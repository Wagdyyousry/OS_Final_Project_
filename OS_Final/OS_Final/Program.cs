...
namespace OS_Final
{
    ...
    class Virtual_disk
    {
        ...
        public void Prepare_Fat_Table()
        {
            Console.WriteLine("Welcome to MINI_FAT Project");
            Console.WriteLine();
            Console.WriteLine("Developed by Wagdy Yousry & Ahmad Abdalraouf");
            Console.WriteLine();
            string path = Directory.GetCurrentDirectory() + @"\\Virtual_disk.txt";
            FileInfo Virtual_disk_txt = new FileInfo(path);
            directory root = new directory("C ", 1, 5, 0, null);
    ...
    public class directory : DirectoryEntry
    {
        ...
        public void write_directory()
        {
            ...
            for (int i = 0; i < num_of_req_block; i++)
            {
                ...
                MINI_FAT.setNext(fc, -1);
                if (lc != -1)
                {
                    MINI_FAT.setNext(lc, fc);
        ...
        public void delete_directory()
        {
            ...
            {
                index = firstCluster;
                next = MINI_FAT.getNext(index);


                do
                {
                    MINI_FAT.setNext(index, 0);
                    index = next;
                    if (index != -1)
                    {
                        next = MINI_FAT.getNext(index);
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Prepare_Fat_Table(commands, details, number_of_arguments);
            Virtual_disk disk1 = new Virtual_disk();
            disk1.Prepare_Fat_Table();
            for (int i = 0; i < c_dic.Fname.Length && c_dic.Fname[i] != '\0'; i++)
            {
                c_pos += c_dic.Fname[i];
            }
            Console.WriteLine(MINI_FAT.getAvaliablIndex());
            Console.WriteLine(MINI_FAT.getAvaliableBlock());
            // MINI_FAT.testing();
        ...
        static void Prepare_Fat_Table(string[] commands, string[] details, List<KeyValuePair<int, int>> number_of_arguments)
        {
            commands[0] = "cd";
                ...
                if (che)
                {
                    ...
                    else if (commands[index] == "md")
                    {
                        ...
                        if (pos == -1)
                        {
                            ...
                            MINI_FAT.setNext(d.firstCluster, -1);
                    ...
                    else if (commands[index] == "dir")
                    {
                        ...
                        Console.WriteLine("     " + dir_no + " Dir(s)" + "     " + (MINI_FAT.get_free_space() - fl_size) + " Free bytes");
                    ...
                    else if (commands[index] == "rd")
                    {
                        ...
                        Console.WriteLine("This Command  not vaild ");
                }
